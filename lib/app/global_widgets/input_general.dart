...
class InputGeneral extends GetView<RegistrarUsuarioController> {
  InputGeneral(
      {super.key,
      this.validator,
      this.padding,
...
class SelectMultiState extends State<SelectMulti>
  ...
  List<Widget> _buildListSelect() {
    ...
    return List.generate(10,
        (index) => _buildSelect(SelectMultiType('Pista $index', value: index)));
  }
  ...
  Widget _buildCheck(SelectMultiType selectMultiType) {
    return BtnIcon(
      ...
      buttonSize: 30,
      fillColor:
          selectMultiType.changeCheck.isTrue ? Colores.verde : Colors.white,
      icon: const Icon(
        Icons.check,
        color: Colors.black,
        size: 15,
